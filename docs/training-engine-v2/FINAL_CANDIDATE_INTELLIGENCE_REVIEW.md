...
Status: **TYPES_VALIDATION_OBSERVABILITY_IMPLEMENTED**.
...
Validation and `buildTrunkMechanicsTrace` are the only new consumers. Strong mechanics cannot manufacture role eligibility, an absent profile emits explicit unavailable/unknown trace evidence, and notes/provenance are non-behavioral. Existing reference exercises received no role or profile changes.
