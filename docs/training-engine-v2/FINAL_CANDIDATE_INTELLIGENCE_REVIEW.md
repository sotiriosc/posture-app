...
Recommended future boundary: contextual annotation-only phase preference downstream of hard eligibility, accepted role/section scope and complete provenance, zero production influence for `needs_review`/unknown/conflicting evidence, removal of both duplicated mechanical bonuses, and explicit omission of phase influence for unknown/conflicting evidence. Final gaps and weight remain unselected pending the deterministic calibration lab.

Remaining P1 work includes contextual-phase resolver implementation, human curation of accepted contextual annotations with complete provenance, deterministic phase calibration, owner resolution of the 17 trunk-mechanics needs-review proposals before additional profiles or trunk assessment features, and support/stance schema implementation before proposed trunk/carry candidates can become production metadata. Overall readiness remains TARGETED_FIXES_REQUIRED_BEFORE_SESSION_COMPOSITION.
...
| shoulder push, mild/current discomfort | push-up / 7.579 | machine-chest-press / 7.409 | none | PAIN_REQUIRES_REVIEW; pain=7.30; joint=8.00 | GOOD |
| low-back hinge, moderate pain | cable-pull-through / 7.525 | dumbbell-romanian-deadlift / 7.338 | none | PAIN_REQUIRES_REVIEW; pain=6.40; joint=7.40 | PLAUSIBLE_NEEDS_REVIEW |
| knee squat, historical sensitivity | goblet-squat / 7.965 | leg-press / 7.948 | none | PAIN_REQUIRES_REVIEW; pain=7.40; joint=8.10 | GOOD |
...
| beginner-gym-shoulder-concern | beginner | commercial_gym | final-horizontal-push-main | machine-chest-press / 7.827 | 3 | assessment affects warmup and main work; mild discomfort is not a blanket hard gate |
...
- Deterministic phase calibration lab execution and owner selection of category spacing/phase-family weight, followed by mechanical-bonus removal and full revalidation before Session Composer.
...
- Truthful support/stance schema implementation and exact seven-exercise production metadata before any new trunk/carry reference exercises are implemented.
...
The engine is not classified READY merely because tests are green. Legal candidate pools are truthful, deterministic ranking and DecisionTrace are strong, progression/replacement semantics are separated, the first trunk profiles remain observability-only, and the structured prescription/progression contract is ready without production calibration. Contextual phase evidence still needs resolver implementation, accepted-provenance curation, calibration and full coefficient revalidation; the remaining trunk-mechanics proposals need owner resolution before assessment expansion; and the owner-curated trunk/carry catalog needs truthful support/stance representation before production implementation. The correct state remains targeted fixes before composition.
