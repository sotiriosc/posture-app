...
- knowledge-compatible seven-row production regression covering exactly 37 canonical rows, seven stable IDs, complete owner provenance, the three explained contextual winner changes, no continuity winner change, contextual omission semantics, intrinsic-versus-potential stress, truthful role/section pools, compact fallback coaching, no second catalog/UI/Knowledge dependency, and isolated before/after fingerprints;
- whole-body candidate-intelligence audit covering all 37 rows, 44 need archetypes across ten explicit equipment environments, role purity, primary-versus-secondary muscle truth, direct muscle-only requests, pain/support/progression boundaries, owner-only P0/P1 proposals, report/source consistency, ten separate audit fingerprints, and unchanged production behavior;
...
Current status supersedes the historical pre-implementation wording above: owner decisions and exactly seven rows are implemented, taking the canonical catalog from 30 to 37. The contextual scorer is production authority after exact root-cause vectors, non-phase invariance, continuity review, and the seven-case omission audit passed the revised semantic gate. The separately authorized whole-body audit is now part of this suite and classifies Candidate Intelligence as `TARGETED_DOMAIN_AND_CATALOG_FIXES_REQUIRED_BEFORE_COMPOSITION`; it changes no production row or behavior. Session/Week Composer and implementation of the proposed role, muscle-contribution, or catalog changes remain outside this suite.
