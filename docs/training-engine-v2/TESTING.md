...
Current status supersedes the historical pre-implementation wording above: owner decisions and exactly seven rows are implemented, taking the canonical catalog from 30 to 37. The contextual scorer is production authority after exact root-cause vectors, non-phase invariance, continuity review, and the seven-case omission audit passed the revised semantic gate. Full build, full unit suite, controlled scenarios, golden personas, continuity dual-run, catalog validation, deterministic fingerprints, and existing PR checks remain required. Session/Week Composer and whole-body audit remain outside this suite.
