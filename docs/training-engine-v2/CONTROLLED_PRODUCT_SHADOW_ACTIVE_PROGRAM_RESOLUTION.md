# Controlled Product Shadow Active Program Resolution
...
Server state resolves a current non-deleted Program deterministically. Because browser AppState is not persisted in the snapshot, a client anchor can verify or conflict but can never override server resolution.
