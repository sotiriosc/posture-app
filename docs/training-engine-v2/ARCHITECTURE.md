...
- `phaseSuitability.ts`: deterministic role/section contextual phase resolution, conflict handling, accepted-provenance qualification, runtime annotation validation, and the explicit non-default `CONTEXTUAL_ANNOTATION_ONLY_LOW_CHURN` scorer. Legacy production scoring remains active until accepted annotation coverage is approved and activation is explicit.
...
Candidate Intelligence does not generate workouts, compose sessions, compose weeks, run beam search, define final phase gates, or connect to Praxis application code. Candidate ranking is evidence for later composition, not a program. The response receiver and progression integration are policy/observability boundaries; they select no dose axis or exercise. The engine input/result safety contract is complete, while golden-product adapter work is separately `PRODUCT_ADAPTER_PENDING`. The owner-selected contextual phase scorer is non-default because accepted contextual annotation coverage remains pending. The focused current-row and seven-row stress/support curation is ready for owner decisions, but no catalog row or whole-body audit is authorized.

Current implementation update: owner phase/stress decisions are applied and exactly seven rows are present once each in the canonical 37-row catalog. The contextual scorer remains non-default because dual-run inspection found three winner changes whose new winner had no accepted phase component; legacy phase scoring remains authoritative. The future Praxis Knowledge Layer is an optional adapter keyed by `ExerciseDefinition.id`, not an engine dependency. Praxis Library, Coaching Rail, UI, CMS, routes, Session/Week Composer, and the whole-body audit remain unimplemented.
