...
- `sessionComposer/contracts.ts`: needs-first intent, immutable Candidate Intelligence handoff, composition result, sequencing result, and owner-boundary contracts.
- `sessionComposer/lab.ts`: bounded deterministic non-production set-cover and sequencing laboratory; no production workout generation or prescription.
...
The Session Composer design phase is authorized. The package now exposes needs-first design contracts and a deterministic laboratory. Session Intent declares actual required and optional needs; Candidate Intelligence remains authority for legal candidates per exact need; composition chooses the smallest complete legal set and compares equal-size alternatives for optional overlap, useful continuity, redundancy, fatigue, setup and candidate evidence; sequencing separately honors explicit dependencies; Prescription remains authority for realized dose and stress.

The laboratory is deliberately named `runSessionCompositionLab` and bounded to small snapshots. It is not a public production `composeSession` orchestrator, does not call Candidate Intelligence, and does not connect to product state. Display sections create no quotas. Infeasibility remains visible rather than causing fabricated coverage.

See `SESSION_COMPOSER_ARCHITECTURE.md`, `SESSION_COMPOSER_WHOLE_SESSION_LAB.md`, and `SESSION_COMPOSER_IMPLEMENTATION_CONTRACT.md`.
