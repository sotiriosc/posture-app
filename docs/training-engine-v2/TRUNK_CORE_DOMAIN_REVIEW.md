...
Its classification remains `TRUNK_CARRY_CONTRACT_FIXES_REQUIRED`; the proposal authorizes no exercise. The equipment dependency is resolved as `TRUNK_CARRY_EQUIPMENT_CONTRACT_READY`, and the structured prescription/progression dependency is resolved as `STRUCTURED_PRESCRIPTION_AND_PROGRESSION_CONTRACT_READY`; both contracts exist without changing any current exercise requirement or behavior. The provisional wall-supported march still awaits owner curation, not representational side/dose semantics. The next dependency is trunk/carry pain-stress vocabulary and receiver review, followed by exact seven-exercise owner curation and production metadata. The 17 qualified secondary-mechanics proposals remain independently unresolved.
...
8. Resolve trunk/carry pain-stress vocabulary and receiver review before any new stress tags or pain-response requirements are added.
9. Complete exact seven-exercise owner curation and production metadata only after pain-stress review; resolve the 17 needs-review proposals before additional profiles or normalized trunk assessment features.
