...
Independent `TrainingSafetyState` is not a score input and never poisons candidates. Candidate ranking remains available for diagnostics, while result-level `TrainingReadinessTrace` marks ordinary downstream execution unavailable for explicit unresolved review authority. Pain severity, region, posture, form, distribution, stress tags, and exercise identity cannot manufacture this authority.

`TrainingResponseHistory` is also not a score component. The deterministic response receiver now routes exact current evidence to prescription review, progression readiness and continuity before any replacement consideration. Related realizations expose structured differences and identity-only history remains weaker background. No receiver output changes candidate score, creates a permanent penalty, selects progression, or automatically replaces an exercise.

The owner-selected future policy is `CONTEXTUAL_ANNOTATION_ONLY_LOW_CHURN`: accepted contextual annotations use excellent `8.8`, good `7.8`, possible `6.2`, poor `5.5`, with phase-family weight `1.0`. Needs-review, unknown, no-match and conflict omit `phase_fit` and its denominator weight. The non-default scorer contains no Phase 1 skill/stability or Phase 3 loadability bonus. Legacy production `phase_fit` remains active until truthful accepted annotation coverage receives owner approval and activation is explicit.
