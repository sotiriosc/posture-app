...
| loaded_spinal_flexion | PLAUSIBLE_NEEDS_REVIEW, OVERBROAD | one-arm-dumbbell-row [loading.jointStressTags + cautionStressTags], dumbbell-romanian-deadlift [loading.jointStressTags + cautionStressTags + contraindicatedStressTags] | cautionStressTags, contraindicatedStressTags, loading.jointStressTags | Load and range modifiable; exact flexion direction should be reviewed per exercise. | General enough to be useful, but current mappings are broader than controlled crunch work. Side: Usually bilateral or midline; side may be irrelevant unless a future prescription creates asymmetry. | REVIEW_LATER |
| loaded_spinal_extension | PLAUSIBLE_NEEDS_REVIEW | dumbbell-shoulder-press [loading.jointStressTags + cautionStressTags], glute-bridge [loading.jointStressTags + cautionStressTags] | cautionStressTags, loading.jointStressTags | Load, range, and setup dependent. | General but needs field-level provenance because current uses span shoulder press and glute bridge. Side: Mostly midline unless future unilateral loading creates asymmetric extension demand. | REVIEW_LATER |
...
| --- | --- | --- | --- | --- | --- | --- |
| sustained upper-limb support loading | sustained_upper_limb_support_loading | ACCEPT_FOR_OWNER_DECISION | forearm-plank, forearm-side-plank | Intrinsic to the reviewed forearm-support identities; magnitude is support, lever, duration, and effort modifiable. | Pain matching for shoulder/elbow support sensitivity and explicit hard/acute support restrictions. | Do not count it as anti-extension, lateral trunk loading, or long-lever core. |
...
| --- | --- | --- | --- | --- | --- | --- |
| forearm-plank | shortened/support-regressed | sustained_upper_limb_support_loading | duration, effort, support | none | long_lever_core | none |
| forearm-plank | ordinary full lever | sustained_upper_limb_support_loading | duration, effort | long_lever_core | none | none |
| forearm-plank | lengthened lever | sustained_upper_limb_support_loading, long_lever_core | lever, duration, effort | none | none | whether a longer-lever tag split is needed later |
| forearm-side-plank | bent-knee/support-regressed | sustained_upper_limb_support_loading | lateral_trunk_loading, duration, support | none | none | whether lateral_trunk_loading can be removed under high support |
| forearm-side-plank | full lever | sustained_upper_limb_support_loading, lateral_trunk_loading | lever, duration | none | none | long_lever_core double-count policy |
| forearm-side-plank | externally loaded future state | sustained_upper_limb_support_loading, lateral_trunk_loading | load, lever, duration | possible grip_loading if held implement is introduced | none | whether external loading belongs in same exercise identity |
