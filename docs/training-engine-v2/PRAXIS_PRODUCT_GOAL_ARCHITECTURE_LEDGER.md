...
- **Status:** completed and proven; `STANDARD_COMMERCIAL_GYM_FOUNDATIONS_AND_INCLINE_PRESS_REALIZATION_V1_IMPLEMENTED_NOT_PRODUCT_ACTIVATED`
- **Authorization:** `STANDARD_COMMERCIAL_GYM_FOUNDATIONS_AND_INCLINE_PRESS_REALIZATION_V1`
- **Owner-selected package:** `PACKAGE_S_STANDARD_COMMERCIAL_GYM_FOUNDATIONS_V1`; exactly eleven common standard-commercial-gym identities are admitted with complete Knowledge and truthful equipment, support, mechanics, stress, and progression metadata
- **Implementation commits:** incline/machine/assistance foundation `b1f61f800562a5c44505c1e9e5fa4952d509071c`; Package S Knowledge core `aade9b8cdf894dffad30aa3c71aefcb9237859a5`; Package S production admission `5dec97de66f91c818fa2e1444f58fa69f0359026`; ledger closure is this ledger-only commit
...
- **Ontology classification:** `STANDARD_GYM_FOUNDATIONS_INCLINE_PRESS_ONTOLOGY_READY`; one canonical exercise catalog and one canonical Knowledge registry remain authoritative, and no Product runtime Knowledge dependency or second catalog is introduced
- **Catalog transition:** 53 to 64 rows and 53 to 64 unique canonical IDs; eleven selected additions, zero unselected additions, zero duplicate IDs, and zero incline-specific duplicate identities
- **Selected IDs:** `pull-up`, `hack-squat`, `seated-leg-curl`, `machine-chest-fly`, `machine-hip-adduction`, `machine-hip-abduction`, `seated-calf-raise`, `machine-hip-thrust`, `cable-lateral-raise`, `overhead-cable-triceps-extension`, and `straight-arm-cable-pulldown`
- **Incline dumbbell decision:** flat and incline are typed realizations of the single canonical `dumbbell-bench-press` identity under `PRESS_SUPPORT_ANGLE_REALIZATION@1.0.0`; flat requires a flat bench, incline requires an adjustable or fixed-incline bench, optional exact degrees do not create an identity, explicit incline preference may choose an available incline realization, automatic angle rotation is disabled, and no upper-chest outcome is guaranteed
- **Incline machine decision:** flat and incline are typed realizations of the single canonical `machine-chest-press` identity; incline requires exact brand-neutral `incline_chest_press` capability, no gym label infers it, and selectorized or plate-loaded mechanisms are accepted only where the exact capability and equipment profile agree
- **Pull-Up decision:** one canonical `pull-up` identity owns unassisted and machine-assisted realizations under `PULL_UP_ASSISTANCE_REALIZATION@1.0.0`; assistance has dedicated apparatus and magnitude truth while external load remains null. Automatic assistance progression, band-assisted Pull-Up admission, and Chin-Up admission are all deferred
- **Machine capabilities:** exact brand-neutral IDs advance from nine to eighteen through `assisted_pull_up`, `incline_chest_press`, `hack_squat`, `seated_leg_curl`, `chest_fly`, `hip_adduction`, `hip_abduction`, `seated_calf_raise`, and `hip_thrust`; brand/model IDs, gym-label inference, and machine-is-safer claims remain zero
- **Knowledge completeness:** canonical production Knowledge advances from 53 to 64 complete entries and from 664 to 835 facts; realization overrides advance from 28 to 41. Every selected identity has accepted `focus`, `cues`, `setup`, `during`, `pattern`, and `watchFor` categories, with zero missing categories, duplicate facts, orphan facts, unresolved claims, diagnosis claims, or treatment claims
- **Compact fallback:** all 64 production fallbacks are generated from canonical Knowledge with zero stale rows; the prior 53 projections remain byte-for-byte equivalent to their frozen baseline
- **Candidate and Composer consequences:** all eleven rows enter only through truthful legality, purpose, role, mechanics, stress, and exact equipment capability; `shoulder_extension` is added for Straight-Arm Cable Pulldown, which remains accessory rather than a required vertical-pull substitute. Knowledge affects ranking and numeric policy by zero, and no fake squat, pull, substitution, or downstream rescue is accepted
- **Week and Prescription consequences:** every selected row has explicit phase abstention, repetition-set Prescription support, reviewed progression and transitions, and deterministic coexistence evidence. Optional bloat, duplicate session identity, automatic angle rotation, and automatic assistance reduction remain zero
- **Standard-gym comfort:** `STANDARD_GYM_COMFORT_PROFILE@1.0.0` covers all 64 rows and is a late deterministic preference only when a comparison contains a Package S machine profile; exact productive familiarity remains higher precedence, while Safety, legality, purpose/coverage, pain/response truth, and dependencies remain authoritative
- **Remaining gaps:** barbell bench press and back squat are useful after owner delivery; conventional deadlift, trap-bar deadlift, preacher curl, and assisted dip require identity review; Smith pressing/squatting and suspension row require equipment contracts; machine lateral raise is useful before public activation; machine biceps curl, machine triceps extension, and cable hip adduction/abduction are redundant with current identities; back extension and power/ballistic equipment require purpose policy. None is required for truthful owner Get stronger delivery from the 64-row catalog
- **Product/runtime boundary:** Product Shadow, current Product UI, goal options, Get stronger visibility, Questionnaire, persistence, `generateProgram`, delivered Program, consumer/gyms behavior, Coaching Rail, Library routes/UI, owner delivery, and Product activation changes: 0. Full / Lighter / Recovery implementation count is 0
- **Evidence:** 576 controlled scenarios; 660 fixed-shell cohort cases; frozen 960-case holdout; 57/57 semantic mutations rejected; 21/21 metamorphic relations passed; deterministic stress failures 0; catalog, Candidate, Composer, Week, Prescription, incline, Pull-Up, standard-gym comfort, Product Shadow, Product/Get stronger, and activation-guard evidence passed
- **Fingerprints:** selected IDs `dc919244db01cf7033501f45a0c8bdbb9b5dc9335a67630a52f244de2162d6e1`; selected rows `f7b198833f1ef5c23944e598ab5bbaec4fd59b52eab9c3b8cd80172a54ecd0f6`; production Knowledge `f3138efd15799c441d07e5893763bb52849494d652d59780555bf1b08139f246`; compact fallback `0d29118d7642cea006fdb1168d9bcbcf1032e69c03169b29b43f256be36740b2`; press-angle contract `be7dab8231adcc5245c9f3be73ae5c5f31008ec68dd59e2ecc932fb25c2e754d`; assistance contract `127517f92ce796c7df7e5c9ac77bb4f2fafa0495fe5ed615e5b07677517450ff`; standard-gym comfort `61fe9f1211b14e707d2a4c13ae876992019ea88521f4862b2565bfb355bcd3b1`; holdout `9a3d2ceb62d9d186ec81abcc53572f0e6eff79ca3ea2a3668dc3b64465729ce5`; activation guards `17e3ae908214b6c676e0e44441729c2e85ac8eb211db83e6d0bbc64771da9939`; combined Pre-G2L `5926959c3540ed3045ad7ab55cdcbde516be85b73bb7fac1ed572982a1b5a267`
- **Preserved upstream fingerprints:** all B1-F, Pre-G1, Pre-G2, and Pre-G2K fingerprints remain authoritative, including combined Pre-G2K `400ca72d6572af9bdec78140e7e5aadfae334e79c38ba0cc0f9a354a5ca71593`; canonical ledger before Pre-G2L `e5fdf6603fa2399017aa692fcf654b45cb740d63bb25637080cc9e308692a361`
- **Future boundary:** Pre-G3, G, and H remain open; Full / Lighter / Recovery, owner delivery, and Product activation are not authorized. The canonical ledger's final state remains incomplete because future work remains
- **Rollback boundary:** revert this ledger-only closure, then revert Package S production admission `5dec97de66f91c818fa2e1444f58fa69f0359026`, Knowledge core `aade9b8cdf894dffad30aa3c71aefcb9237859a5`, and incline/machine/assistance foundation `b1f61f800562a5c44505c1e9e5fa4952d509071c` in that order; the exact 53-row, 53-entry Pre-G2K baseline remains the fallback without Product data or route migration
- **Next dependency:** `SESSION_PRACTICE_OPTIONS_FULL_LIGHTER_RECOVERY_V2_BRIDGE_AUTHORIZATION`

## Pre-G3 — Full / Lighter / Recovery V2 day-of practice-options bridge

- **Status:** completed and proven; `SESSION_PRACTICE_OPTIONS_FULL_LIGHTER_RECOVERY_V2_BRIDGE_IMPLEMENTED_NOT_PRODUCT_DELIVERED`
- **Authorization:** `SESSION_PRACTICE_OPTIONS_FULL_LIGHTER_RECOVERY_V2_BRIDGE_IMPLEMENTATION_NOT_OWNER_DELIVERY_OR_PRODUCT_ACTIVATION`; owner-account delivery and Product activation are not authorized
- **Implementation commits:** V2 domain and purpose-preserving policies `473b874a93936343b2260d4dfa3f10bf9f1dea13`; Prescription, final Sequencing, Gate 13, completion, and downstream evidence `68f6434cd653d12c0f96bd5791bbbb9a2c118d0a`; default-off server persistence and future Product adapter `b41cff82b5a3bb18e4e9a9f9d0de84abe412f4d9`; ledger closure is this ledger-only commit
...
- **Ontology classification:** `SESSION_PRACTICE_OPTIONS_V2_ONTOLOGY_READY`; request, recommendation, availability, selection, realization, completion, persistence, and downstream evidence are separate typed facts
- **Historical V1 freeze:** `packages/engine/src/sessionPracticeOptions.ts` remains byte-exact at SHA-256 `988c8e62bc0975662d6b505d2080d00bacc16494f3f87fe4ebf7f14b05f23e6c`; `apps/consumer/src/app/session/SessionClient.tsx` remains byte-exact at SHA-256 `35107e225e8e2b5f572677d55dbaa4c00957d317990d6cf4b96467c0cb7d41fa`; current Full/Lighter/Recovery copy, DOM, selection, reset, resume, and completion behavior remain V1
- **Contracts:** `HISTORICAL_SESSION_PRACTICE_OPTIONS_V1@1.0.0`, `SESSION_PRACTICE_OPTIONS_V2_BRIDGE@1.0.0`, `SESSION_PRACTICE_REQUEST@1.0.0`, `SESSION_PRACTICE_OPTION_POLICY_V2@1.0.0`, `SESSION_PRACTICE_OPTION_AVAILABILITY@1.0.0`, `SESSION_PRACTICE_REALIZATION_PLAN@1.0.0`, `SESSION_PRACTICE_REALIZATION_REVISION@1.0.0`, `SESSION_PRACTICE_ASSIGNMENT_DISPOSITION@1.0.0`, `SESSION_PRACTICE_COMPLETION_DISPOSITION@1.0.0`, `SESSION_PRACTICE_OUTCOME_SOURCE_LINK@1.0.0`, `SESSION_PRACTICE_PERSISTENCE@1.0.0`, `SESSION_PRACTICE_PRODUCT_ADAPTER@1.0.0`, `SESSION_PRACTICE_OBSERVABILITY@1.0.0`, and `PRODUCTION_SESSION_PRACTICE_REALIZER_KERNEL@1.0.0`
- **Request and availability:** one explicit athlete/day-of request is bound to one exact source session revision; suggestion never auto-selects; all three options expose typed availability and reason codes; training-safety blocks all modes
- **Full policy:** `SESSION_PRACTICE_FULL_POLICY_V1_EXACT_PASS_THROUGH@1.0.0`; assignments, exercise identity, dose, rest, order, dependencies, purpose, and source lineage are semantically exact, with Full semantic-difference count 0
- **Lighter policy:** `SESSION_PRACTICE_LIGHTER_POLICY_V1_STRUCTURAL_FIRST@1.0.0`; deterministic L0-L6 reduction removes redundancy before primary work, preserves required responsibility and protected anchors, changes no exercise identity, repetitions, rest, or guessed load, applies only explicit Prescription bounds, and fails closed when no materially lighter valid realization exists
- **Recovery policy:** `SESSION_PRACTICE_RECOVERY_POLICY_V1_EXPLICIT_SUPPORT_ONLY@1.0.0`; availability requires explicit structured recovery/support ownership, never keyword parsing, and grants zero developmental responsibility or original-day completion credit
- **Dependency pruning:** omitted work is explicit, and only orphaned dependencies are removed after transformation; required dependencies retained by remaining assignments remain present
- **Lifecycle and attempt identity:** deterministic request, attempt, realization, and revision identities preserve source lineage; one attempt has one `final_for_execution` realization, mode changes lock after execution starts, and no progress is silently cleared
- **Prescription and final Sequencing:** every modified dose creates an explicit Prescription revision based on the source revision; the final sequence is deterministically rebuilt after transformation by section, dependency, and block order, introduces no pairing, and preserves unknown duration as unknown
- **Realization and burden trace:** each plan records assignment dispositions, retained anchors/responsibilities, omitted work, dependency pruning, first meaningful difference, burden changes, source fingerprints, and all contract versions
- **Completion and Gate 13:** completion is derived from realized source events rather than the original day; omitted assignments are never performed; Lighter and Recovery do not automatically complete the day; Gate 13 validates the exact final realization, source events, and remaining responsibility
- **Outcome Source:** completed realized work can emit one idempotent source event with exact athlete, session, source revision, attempt, realization revision, and completion lineage; duplicate source events are rejected
- **Longitudinal and remaining-Week boundaries:** practice evidence is observation-only; remaining responsibility is an explicit no-action handoff. Automatic reallocation, progression, regression, deload, replacement, Week mutation, and Longitudinal application counts are 0
- **Persistence and replay:** server-only append-only persistence stores immutable athlete-scoped attempt revisions; exact retry is idempotent, semantic conflict and missing lineage fail closed, exact revision retrieval has no latest fallback, draft resume restores exact mode/position/performance/timers/substitutions, stale source revisions conflict, and current Product records are not mutated. PostgreSQL migration `001_session_practice_v2_attempt_revisions` is isolated and checksum-locked at `65e46f0d4dd0bd45ab3e90926f2c51af13a60099ac860e772104783ef7658886`
- **Future Product adapter:** exported only as `@praxis/engine/session-practice-v2`, default off, preserves exact current V1 labels, emits no Product output, writes no Product persistence, runs no Product Shadow profile, and is not imported by the current engine root or SessionClient
- **Observability:** structured identifiers, modes, reasons, availability, lifecycle, revisions, completion, and replay status only; prohibited prose, medical, photo, hidden Product, and current-route event fields count 0
- **Registry V17:** `CAGT_EFFECTIVE_AUTHORITY_REGISTRY@17.0.0` preserves V16, records athlete request and production-kernel-not-Product-runtime authorities, keeps recommendation suggestion-only, Product decision authority legacy-only, Product activation unauthorized, owner delivery false, and production imports 0; fingerprint `aaa63e96a3b65ac0225c84d238943ada60c556e1646388b055ee2ea4f6481ed7`
- **Catalog and Knowledge coverage:** all 64 unique production identities and all 64 complete Knowledge entries are exercised across all seven dose modes and five session sections; Pre-G3 adds no catalog rows or Knowledge entries
- **Evidence:** 720 controlled scenarios; 1,030 fixed-shell cohort cases; frozen 1,200-case holdout with 400 cases per mode, 200 historical V1 goldens, 1,000 genuine V2 realizations, 300 Gate 13 cases, and 240 persistence/replay cases; 60/60 semantic mutations rejected with zero downstream rescues; 11/11 invariant and 10/10 material-response metamorphic relations passed; all mandated deterministic stress floors passed with zero failures
- **Activation guards:** historical V1, current option copy, current route DOM, SessionClient V2 imports, current Product route calls, current Product persistence, ProgramProgress, SessionRecord, Product Shadow, Product UI, Get stronger visibility, owner delivery, and Product activation changes are all 0; G and H remain open
- **Fingerprints:** contracts `19a9131c37d7037148ba84fbdb6bdafaddafdd00469be95ed0386bcb83086157`; historical V1 `988c8e62bc0975662d6b505d2080d00bacc16494f3f87fe4ebf7f14b05f23e6c`; controlled scenarios `8ff91fd3dd254f803931c1c6705b14911b8cb1265205e5f23803338800056072`; cohorts `9793ecfeda24a7792d64b744c4e972b126ef50dcbbfec94723a45be592c449a9`; holdout `3c34bb4a6f9a9648bcb701067e16fdbfd53dd583510b6cdcd9a9917889726ce5`; mutations `fbc172d1327b9d990c1d8ce799b569bbd44cd323613b2996bdaf9a50715321b7`; metamorphic `d752d34dcd36fe062b686ad4875ac519545e53be680a34d7f71ee99c833018fc`; stress `4bdcae13458a4602efd84cefebfe162c92076ecb4e63b33becc25dfeeca51d06`; activation guards `3a83ced409f4d2dab485886306649fbdee91ebac80ab3fced98536f83b95f1b6`; persistence `cd48982b58bc1ad7f00daad7aa0061d07fa4e944a9b782fe2d26ea4b6570c6fb`; combined Pre-G3 `0a61f586fb885c32fe5cf64a906d527b985c41127ad69cb4a1e5952b23dfec38`
- **Preserved upstream fingerprints:** all B1-F, Pre-G1, Pre-G2, Pre-G2K, and Pre-G2L fingerprints remain authoritative, including combined Pre-G2L `5926959c3540ed3045ad7ab55cdcbde516be85b73bb7fac1ed572982a1b5a267`; canonical ledger before Pre-G3 `ab1d2e20ca25b9e8778ef0945501dcdd0a7faef1b9afcb742b2266e9744ff0c9`; the post-closure ledger SHA-256 is publication metadata reported after this self-referential ledger is committed
- **Readiness:** Pre-G3 is implemented and proven but not Product-delivered; G controlled owner-account delivery and H broader activation remain separately authorized. The canonical final state remains incomplete because future work remains
- **Rollback boundary:** revert this ledger-only closure, then revert default-off server bridge `b41cff82b5a3bb18e4e9a9f9d0de84abe412f4d9`, realization integration `68f6434cd653d12c0f96bd5791bbbb9a2c118d0a`, and pure policies `473b874a93936343b2260d4dfa3f10bf9f1dea13` in that order; V1 remains the current Product path and the isolated V2 table can remain unread or be removed before any future activation because it contains no current Product rows
- **Next dependency:** `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_GOAL_DELIVERY_DESIGN_V1_AUTHORIZATION`

## Chunk G — Controlled owner-account delivery

### G Design — Controlled owner-account Get stronger delivery

- **Status:** design completed and proven; `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_GOAL_DELIVERY_DESIGN_V1_COMPLETED_NO_OWNER_DELIVERY`. G implementation remains open and separately authorized
- **Authorization:** `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_GOAL_DELIVERY_DESIGN_V1_NOT_DELIVERY_IMPLEMENTATION_OR_PRODUCT_ACTIVATION`; no live owner-account read, deployment-environment mutation, owner delivery, live V2 Program generation/application, or Product activation occurred
- **Design commit:** `2f9213948f9cf7527f898cfd7e78d0235b3a8c54`; ledger closure is this ledger-only commit
...
- **Ontology:** `CONTROLLED_OWNER_GET_STRONGER_DELIVERY_ONTOLOGY_READY`; authentication, identity eligibility, delivery mode, enrollment, profile readiness, generation, preview, approval, application, projection, rollback, monitoring, and activation are separate typed facts
- **Configured owner identity:** redacted server reference `configured_owner_email_reference` sourced only from `AUTH_USER_EMAIL`; literal configured-email count in tracked source, reports, JSON evidence, tests, logs, telemetry, migrations, and this ledger is 0
- **Identity and authorization:** `CONTROLLED_OWNER_ACCOUNT_IDENTITY_POLICY@1.0.0` normalizes by trim/lowercase and requires one exact configured-to-stored email match, then persists stable user ID as owner identity. Eligibility is passive through `readServerSession()` and repository lookup, never `ensureBootstrapUser()` or user mutation. Delivery requires both identity eligibility and a separately configured mode
- **Delivery mode:** future-only `PRAXIS_V2_OWNER_DELIVERY_MODE` has closed values `off`, `preview`, and `apply`; absent or unknown is `off`. Identity alone activates nothing, and no deployment value was created or changed
- **Enrollment and profile:** `CONTROLLED_OWNER_V2_ENROLLMENT@1.0.0` and `CONTROLLED_OWNER_GET_STRONGER_PROFILE@1.0.0` define explicit, revisioned owner confirmation. The selected primary goal is Get stronger, training mode is Develop, secondary goal is absent, and 11 minimum structured inputs cover days/opportunities, minutes, equipment capabilities, pain/context, experience/familiarity, and current availability/safety confirmation
- **Owner-only surface and mapping:** selected Option A uses future dedicated `/account/praxis-v2` routes; ineligible access is 404 and ordinary-route discoverability is 0. Get stronger maps explicitly to canonical `strength` plus `develop`, without changing current Product options, visibility, Questionnaire, routes, or middleware
- **Generation, preview, and approval:** `CONTROLLED_OWNER_GET_STRONGER_GENERATION@1.0.0` invokes the authorized production V2 pipeline only after readiness; current `generateProgram` calls and Product Shadow artifact applications remain 0. `CONTROLLED_OWNER_V2_PROGRAM_PREVIEW@1.0.0` is immutable and non-mutating; `CONTROLLED_OWNER_V2_PROGRAM_APPROVAL@1.0.0` is explicit, CSRF-protected, idempotent, and separate from application
- **Application and ownership:** `CONTROLLED_OWNER_V2_PROGRAM_APPLICATION@1.0.0` is designed as a distinct future event. It never overwrites the legacy Program or progress and advances one explicit active Program pointer only after all apply preconditions pass. `OWNER_V2_PRODUCT_PROGRAM_ENVELOPE@1.0.0` preserves source, contract, profile, generation, preview, approval, application, projection, and rollback lineage
- **Projection and owner surfaces:** the future Product projection is lossy-display-only and cannot become authority; lineage-loss count is 0. Dedicated owner Week and Session route designs consume the V2 envelope, while current SessionClient V2 imports remain 0. Full / Lighter / Recovery delivers only typed Pre-G3 practice realizations and changes no current behavior
- **Fallback, conflict, rollback, and kill switch:** legacy fallback remains authoritative unless an applied V2 envelope is active. Active legacy sessions block application. `CONTROLLED_OWNER_V2_ROLLBACK@1.0.0` changes the active pointer without deleting data. `off` blocks new attempts, and an in-flight attempt must revalidate mode before commit
- **Outcome and Longitudinal:** Performance/Outcome and Longitudinal remain observation/proposal boundaries. Automatic progression, regression, and deload counts are 0; no owner-delivery design fact is adaptation authority
- **Monitoring, privacy, security, and billing:** structured IDs, revisions, modes, states, reasons, latency, and failure classes are allowed; email logging and free-text telemetry counts are 0. Server-only eligibility, CSRF, idempotency, athlete scoping, least-data records, and redacted references are required. Billing/plan mutation count is 0 and subscription state is not owner eligibility authority
- **Product Shadow and current Product:** owner delivery and Product Shadow have separate authorities, allowlists, routes, persistence, and artifacts; a Shadow artifact can never be approved or applied. Product, Product Shadow, current routes, Get stronger visibility, Questionnaire, `generateProgram`, current persistence, Full / Lighter / Recovery, and activation remain unchanged
- **State machine and implementation handoff:** `CONTROLLED_OWNER_GET_STRONGER_DELIVERY_STATE_MACHINE@1.0.0` defines 23 closed design states. `CONTROLLED_OWNER_GET_STRONGER_DELIVERY_IMPLEMENTATION_HANDOFF@1.0.0` records the separately authorized future sequence; expected implementation commit count is 5, but implementation and owner-route counts are 0
- **Evidence:** 560 controlled scenarios; fixed-shell cohorts total 720 (`120/100/100/100/100/100/100`); frozen 1,000-case holdout with category counts `300/200/200/150/150`; 50/50 semantic mutations rejected with zero downstream rescue; 9/9 invariant and 11/11 material-response metamorphic relations passed; every mandated deterministic stress floor passed with zero failures
- **Fingerprints:** ontology `b9aac2bbb3dcf22aae55cbb2ed42c03cbf9102bacf7134d03544af60aae75b8a`; auth baseline `1192b36167699da73a4d4aa7f26d752950ac33c20dc9edbaf3dd5558880e6c86`; identity `f04eaf68beaa05cbe4846d6971d7eedfcd6e9161bb3b67d58edeae7486158e70`; state machine `918c40770d593713e26dd4788beed8d6a0f419f7eccf14d9598c7b2f794918ee`; holdout `3640e6adfae7493d70a806e0fbfdae5dff31bb80f8ad35e2d61765de5af92a7a`; report corpus `688b0c437d837d44fd42cdca2c4b686acdb3654a349d0250f7387fcf06d1f533`; activation guards `ba547b97c477641c19aab336651d9a667708272bcf6963a5f80ae2c500d85278`; readiness `8b98ab78e6c4324fe8fffeb026e871d8b348e84db8f06a495148f12af11cd3a2`; combined G design `8b3b4d6f3088d50083498d8885cb19d91e15c56996e3198c7bf6785aa63eef13`
- **Preserved upstream:** B1-F, Pre-G1, Pre-G2, Pre-G2K, Pre-G2L, Pre-G3, catalog `64/64`, Knowledge `64/64`, Product Shadow `fee0ffe0d586123cfd903f01a347f59aa92a8825342d0ec62b86a2235d376e2c`, and combined Pre-G3 `0a61f586fb885c32fe5cf64a906d527b985c41127ad69cb4a1e5952b23dfec38` remain authoritative. Canonical ledger before G design is `efff1d833aad8b84a6c24df4165b67f500aaa885ebd862efd5e1797a23be3bc6`; post-closure SHA-256 is publication metadata reported after this self-referential ledger is committed
- **Validation:** all focused design, auth, identity, route, state, evidence, mutation, metamorphic, stress, guard, report, upstream regression, Training Engine V2, engine, app, browser, build, type, lint, and pure/server-boundary checks pass. PostgreSQL-required suites remain CI-authoritative and skip locally without a database service
- **Remaining limitations:** owner profile persistence and final live inputs, server gate implementation, owner routes, generation/preview/approval/application persistence, Week/Session delivery, live environment setup, explicit owner-consented verification, support/legal review, G implementation, and all H activation work remain open
- **Rollback boundary:** revert this ledger-only closure, then revert design commit `2f9213948f9cf7527f898cfd7e78d0235b3a8c54`; no runtime, environment, Product, owner data, or delivery state requires rollback
- **Next dependency:** `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_GOAL_DELIVERY_IMPLEMENTATION_V1_AUTHORIZATION`

### G Implementation — Controlled owner-account Get stronger delivery

- **Status:** `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_GOAL_DELIVERY_IMPLEMENTED_DEFAULT_OFF_NO_LIVE_OWNER_DELIVERY`; the separately authorized implementation is complete and proven while the delivery mode remains default-off
...
- **Ontology:** `CONTROLLED_OWNER_GET_STRONGER_DELIVERY_ONTOLOGY_READY`; identity eligibility, delivery mode, enrollment, profile readiness, generation, preview, approval, application, active pointer, projection, session practice, Outcome Source, Longitudinal observation, rollback, monitoring, and activation remain separate typed facts
- **Implementation commits:** identity/profile `0e1f82c35b5a51c164dec4256b78a0c40d309786`; generation/application `23ac19a0cbeda09b16c7ab689e2ea08fbf51ae60`; owner preview/Week `a8b73819b268110358570b2cc3ffeebfdcd7a068`; owner Session `8fddb55125bc55ea40fa9dcf2ce80aef79f79335`; Outcome/rollback `0be41bfa726d268dea5c389d294af138ae407fd9`; ledger closure is this ledger-only commit
- **Runtime contract:** the controlled owner runtime is implemented server-side with passive stable-user identity resolution, exact configured/stored/token consistency, explicit enrollment and revisioned profile readiness, and closed `off` / `preview` / `apply` modes. Absent or unknown mode remains `off`; tracked configured-owner literals, email persistence, email logging, `ensureBootstrapUser()` eligibility calls, admin allowlist reuse, and Product Shadow allowlist reuse all remain 0
- **Default-off gate:** all five dedicated owner pages and all 13 owner API routes are implemented but inaccessible while mode is `off`; mode-off and ineligible requests fail closed with 404 and no cache. The kill switch blocks new attempts before session or database access, performs no writes, preserves drafts, and revalidates mode before completion
- **Profile and generation:** explicit enrollment, revisioned structured Get stronger profile, equipment capability, calibration, and unknown-minute handling are implemented. Unknown minutes may be previewed but block approval. The genuine 13-stage production V2 pipeline consumes the canonical 64-identity registry; Product Shadow generation and legacy `generateProgram` calls remain 0
- **Preview, approval, and application:** immutable preview, explicit CSRF-protected approval, idempotent transactional application, and one active V2 pointer are implemented. Approval never auto-applies; legacy Program, Progress, and drafts are not overwritten or cleared; raw Product snapshots are not persisted; mode-off effective ownership remains legacy
- **Owner delivery surfaces:** conditional Account entry plus dedicated profile, preview, Week, Session, and History surfaces are implemented. The owner Session executes typed Full / Lighter / Recovery realizations, exact draft/resume, Performance, completion, and all 64 identities without importing V2 into the current SessionClient
- **Outcome and Longitudinal:** completion appends one idempotent production `session_completion` Outcome Source and one observation-only Session Practice Longitudinal record with exact lineage. Week credit is derived from completed realized work. Automatic progression, regression, deload, replacement, reallocation, and Week rewriting remain 0
- **Rollback:** `CONTROLLED_OWNER_V2_ROLLBACK@1.0.0` requires exact user scope, apply mode, CSRF, idempotency, explicit confirmation, and pointer compare-and-swap; it restores the legacy pointer without deleting the V2 envelope, legacy data, progress, drafts, Outcome Sources, Longitudinal observations, or audit history. Repeated exact rollback is idempotent and an active nonterminal attempt blocks rollback
- **Persistence and PostgreSQL:** isolated append-only owner-delivery tables, checksum-locked migrations, matching in-memory and PostgreSQL repositories, athlete scoping, exact replay, no latest fallback, and transactional application/completion/rollback are implemented. PostgreSQL 16 integration coverage is CI-authoritative
- **Observability, security, and privacy:** bounded structured events contain identifiers, revisions, state, mode, reason, latency, and failure class only. Prohibited telemetry, client identity authority, cross-user reads or writes, billing/paywall mutation, plaintext configured owner identity, and free-text profile logging remain 0; reads and action-specific mutations are rate limited
- **State machine:** all 23 closed states and production transitions declare preconditions, state changes, repository effects, failure behavior, and rollback behavior; wrong-layer acceptance and downstream rescue counts are 0
- **Evidence:** 900 controlled scenarios; 1,140 fixed-shell cases with cohorts `160/140/140/140/140/120/100/100/100`; frozen 1,350-case holdout with categories `250/200/250/200/200/150/100`; 61/61 semantic mutations rejected; 25/25 metamorphic relations passed; all mandated deterministic stress floors passed with zero failures, including 10,000 genuine production generations
- **Fingerprints:** holdout `7165e43b5d7d1f0a5b64e0504cc07e0ccf48956f43b70beafec51e3c901b6242`; stress `91df2544d817d1a84de0e7d20972616a94da636166bb2d037a17e5e25a9d0b53`; combined G implementation `4313af5ea0ad7d9299efb5a9f40a332c71a896dbe9392199e19511f1d0e07d96`
- **Preserved upstream:** B1-F, Pre-G1, Pre-G2, Pre-G2K, Pre-G2L, Pre-G3, G design, catalog `64/64`, Knowledge `64/64`, current Product, Product Shadow, Get stronger visibility, Questionnaire, ordinary Results/Program/Session/Progress/History, gyms, buyer demo, and historical Full / Lighter / Recovery remain authoritative and unchanged. Canonical ledger before G implementation is `62bd62d35dd9d62eda4020b8634a02191c9297607a5b202d1ffe44f6639d442f`; post-closure SHA-256 is publication metadata reported after this self-referential ledger is committed
- **Validation:** controlled owner unit, evidence, holdout, mutation, metamorphic, stress, report-replay, design-regression, Session Practice V2, critical regression, engine build, Training Engine V2 build, consumer build, and focused lint checks pass. Local PostgreSQL tests skip without `TEST_DATABASE_URL`; the required PostgreSQL 16 job runs in CI. Repository-wide lint retains only pre-existing failures in untouched current consumer and gyms components
- **Live boundary:** live environment mutation, live owner-account read, live V2 Program generation, live approval, live application, live owner Session attempt, live owner delivery, Product Shadow artifact application, Product activation, gyms runtime change, and buyer-demo change counts are all 0
- **Remaining work:** owner-only environment enablement, explicit live owner consent and profile confirmation, live preview, live approval/application, live verification, support/privacy/legal checks, and all Chunk H broader activation remain open and require separate authorization
- **Final ledger state:** `INCOMPLETE_FUTURE_WORK_REMAINS`; final ledger completed state is 0, owner live-delivery count is 0, and Product activation count is 0
- **Rollback boundary:** revert this ledger-only closure, then revert Outcome/rollback, owner Session, owner preview/Week, generation/application, and identity/profile commits in reverse order. Leave delivery mode `off`; no live owner or Product state requires repair
- **Next dependency:** `CONTROLLED_OWNER_ACCOUNT_GET_STRONGER_OWNER_ONLY_ENVIRONMENT_ENABLEMENT_AND_LIVE_VERIFICATION_V1_AUTHORIZATION`
...
- remaining separately authorized standard-gym catalog gaps after Pre-G2L;
