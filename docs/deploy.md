...
STRIPE_PRICE_ID_MONTHLY=...    (separate — gyms pricing is not the same product as consumer Pro)
STRIPE_PRICE_ID_ANNUAL=...     (optional for gyms; required for consumer multi-plan checkout)
STRIPE_PRICE_ID_FOUNDERS=...   (optional; else founders uses lookup key praxis_founders_monthly)
