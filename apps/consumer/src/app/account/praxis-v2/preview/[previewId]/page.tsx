...
import { resolveOwnerProgramClassification } from "@praxis/training-engine-v2";
...
export default async function OwnerPreviewPage({ params }: { readonly params: Promise<{ previewId: string }> }) {
  ...
  if (!preview) notFound();
  const programClassification = resolveOwnerProgramClassification(preview);
  ...
  return <OwnerPreviewPresentation preview={preview} styles={styles}
    actions={<OwnerPreviewActions previewId={preview.previewId} previewFingerprint={preview.previewFingerprint}
      programClassification={programClassification} csrf={csrf}
      applyAvailable={gate.mode === "apply" && (preview.readinessStatus === "ready_for_approval" ||
        preview.readinessStatus === "ready_for_initial_calibration_approval")} />} />;
}
