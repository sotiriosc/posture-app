 ...
 * (Phase 6e, Commit 1 / SR-6e) along with everything else, since it has no
 * concept of who's signed in. Left alone, the very next page load's startup
 * check (`syncLocalOwner`, in AppMenuClient) would see a logged-in tester but
 * no marker, read that as a stale/orphaned device, and wipe the data we just
 * seeded a second time. Adopting the current session's owner post-seed —
 * without triggering another wipe — closes that gap; dev-seed already IS the
 * deliberate full reset, so there's nothing left to protect against.
 */
