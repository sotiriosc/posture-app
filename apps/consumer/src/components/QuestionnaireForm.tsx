...
export type QuestionnaireData = {
  ...
  daysPerWeek: 3 | 4 | 5;
  /**
   * Phase 4 — Band setup follow-up. Only meaningful when equipment includes bands.
   * Absent/undefined on legacy stored profiles = legacy_unknown (no anchor/type claim).
   */
...
const normalizeQuestionnaireData = (
  ...
  const equipment = normalizeEquipmentSelectionValues(input?.equipment ?? ["gym"]);
...
export default function QuestionnaireForm() {
  ...
  const toggleArrayValue = (key: "painAreas" | "equipment", value: string) => {
    ...
    if (key === "equipment") {
      ...
      const withoutNone = list.includes("none")
        ? list.filter((item) => item !== "none")
        : list;
      const hasBands = withoutNone.includes("bands");
      updateData({
        equipment: withoutNone,
          ...
          <div className="mt-4 space-y-2" data-testid="band-setup-followup">
            ...
            <div className="grid gap-2">
              {BAND_SETUP_OPTIONS.map((option) => (
                <label
                  key={option.value}
                  className="flex min-h-12 items-center gap-2 rounded-lg border border-slate-500/25 bg-slate-950/45 px-3 py-3 text-sm text-slate-200"
                >
                  <input
                    type="radio"
                    name="band-setup"
