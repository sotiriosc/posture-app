...
- Founders path uses `STRIPE_PRICE_ID_MONTHLY` + silent FOUNDERS promo (no separate founders price)
...
- `STRIPE_PRICE_ID_MONTHLY=price_...` (live monthly)
- `STRIPE_PRICE_ID_ANNUAL=price_...` (live annual)
- Founders path uses `STRIPE_PRICE_ID_MONTHLY` + silent FOUNDERS promo (no separate founders price)
