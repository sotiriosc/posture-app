/**
 * Phase 7B §12 — Gyms presentation parity via shared engine + real interaction wiring.
 */
...
describe("Phase 7B gyms presentation parity", () => {
  it("shared engine presentation fields match gyms contract expectations", () => {
    ...
    const program = generateWeeklyProgram(questionnaire, "p7b-gyms-parity", {
      seed: "p7b-gyms-parity",
    });
    const model = resolveProgramPresentation({ program, questionnaire });

    expect(model.program.equipmentIdentityMode).toBe("gym");
    ...
    expect(model.sessions.every((s) => s.painAdaptation)).toBe(true);
    ...
    expect(resolveNoValidSwapMessage().severity).toBe("safety");
  });
