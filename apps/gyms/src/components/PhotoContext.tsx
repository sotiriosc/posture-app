...
type PhotoProviderProps = {
   ...
   * namespaced per account (Phase 6f, Commit 1 / SR-6f, porting Phase 6e's
   * consumer fix) — set as a server-rendered prop, not discovered
   * client-side, so the very first hydrate on a fresh page load already
   * reads the right account's photos instead of racing a client-side
   * session fetch.
   */
  userId: string | null;
...
export function PhotoProvider({ userId, children }: PhotoProviderProps) {
  // Set synchronously during render, not inside an effect (Phase 6e,
  // Commit 1 / consumer; ported here in Phase 6f, Commit 1). Effects run
  // after commit — too late: a fast interaction right after mount (an
  // automated test's file-input change, or just a quick real user) can
  // reach `setPhoto` before a commit-phase effect has had a chance to run,
  // persisting the photo under the wrong (stale/default) namespace. This
  // module-level assignment doesn't affect this render's output, so doing
  // it in the render body is safe, and it guarantees every descendant
  // (PhotoUploader included) sees the correct namespace from its very
  // first paint.
